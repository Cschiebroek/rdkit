...
//     return coordMap;
// }
const std::map<int, RDGeom::Point3D>& coordmapFromTemplate(const RDKit::ROMol& input_mol) {
...
bool setupInitialBoundsMatrix(
  ...
  if (params.useTemplate  && params.coordMap == nullptr) {
    ...
    // std::map<int, RDGeom::Point3D> coordMap = coordmapFromTemplate(*mol);   
    *coordMap = coordmapFromTemplate(*mol);
  }
...
void EmbedMultipleConfs(ROMol &mol, INT_VECT &res, unsigned int numConfs,
  ...
  std::map<int, RDGeom::Point3D>* coordMap = nullptr;
  ...
  if (params.coordMap) {
      ...
      std::map<int, RDGeom::Point3D> templateCoordMap = RDKit::DGeomHelpers::EmbeddingOps::coordmapFromTemplate(mol);   
